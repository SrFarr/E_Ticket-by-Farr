...
class TransportasiRepo(private val dao: TransportasiDao) {
    ...
    suspend fun searchPesawatByTanggal(startDate: Long, endDate: Long) = dao.searchPesawatByTanggal(startDate, endDate)

    // --- Kereta ---
    suspend fun getAllKereta() = dao.getAllKereta()
    suspend fun searchKeretaByTujuan(tujuan: String) = dao.searchKeretaByTujuan(tujuan)
    suspend fun searchKeretaByTanggal(startDate: Long, endDate: Long) = dao.searchKeretaByTanggal(startDate, endDate)
